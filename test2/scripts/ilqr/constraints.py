...
class Constraints:
	...
	def get_state_cost_derivatives(self, state, poly_coeffs, x_local_plan, npc_traj):
		...
		for i in range(self.args.horizon):
			...
			P1 = np.array([[1], [0],[0],[0]])
			x_lane, y_lane = self.find_closest_point(state[:, i], poly_coeffs, x_local_plan)
			# lane Barrier Max
			c = (np.matmul(state[:, i].T, P1) - self.args.acc_limits[1])
			b_1, b_dot_1, b_ddot_1 = self.barrier_function(self.args.q1_lane, self.args.q2_lane, c, P1)
