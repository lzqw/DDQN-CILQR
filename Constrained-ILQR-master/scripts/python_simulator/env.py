...
from INTERACTION_Sim.INSim import get_item_iterator, draw_map_without_lanelet, polygon_xy_from_motionstate, mapsize, LL2XYProjector, Point,get_type,get_subtype,get_way,get_x_y_lists
...
class Env(gym.Env):
    ...
    def generate_track_dict(self):
        root_dir = sys.path[0]
        dir=[]
        for i in range(len(self.args.track_file_number)):
            dir.append(root_dir+'\\maps\\'+self.args.map_name+'\\tracks\\vehicle_tracks_'+
                       str(self.args.track_file_number[i]).zfill(3) + ".csv")
        track_dict = dict()
        track_id = None
        for diri in dir:
            data=pd.read_csv(diri)
            for i, row in data.iterrows():
                if i == 0:
                    # check first line with key names
                    assert (row.index[KeyEnum.track_id] == Key.track_id)
                    assert (row.index[KeyEnum.frame_id] == Key.frame_id)
                    assert (row.index[KeyEnum.time_stamp_ms] == Key.time_stamp_ms)
                    assert (row.index[KeyEnum.agent_type] == Key.agent_type)
                    assert (row.index[KeyEnum.x] == Key.x)
                    assert (row.index[KeyEnum.y] == Key.y)
                    assert (row.index[KeyEnum.vx] == Key.vx)
                    assert (row.index[KeyEnum.vy] == Key.vy)
                    assert (row.index[KeyEnum.psi_rad] == Key.psi_rad)
                    assert (row.index[KeyEnum.length] == Key.length)
                    assert (row.index[KeyEnum.width] == Key.width)
                    continue
                if int(row[KeyEnum.track_id]) != track_id:
                    track_id = int(row.values[KeyEnum.track_id])
                    assert (track_id not in track_dict.keys()), \
                        "Line %i: Track id %i already in dict, track file not sorted properly" % (i + 1, track_id)
                    track = Track(track_id)
                    track.agent_type = row.values[KeyEnum.agent_type]
                    track.length = float(row.values[KeyEnum.length])
                    track.width = float(row.values[KeyEnum.width])
                    track.time_stamp_ms_first = int(row.values[KeyEnum.time_stamp_ms])
                    track.time_stamp_ms_last = int(row.values[KeyEnum.time_stamp_ms])
                    track_dict[track_id] = track
                track = track_dict[track_id]
                track.time_stamp_ms_last = int(row.values[KeyEnum.time_stamp_ms])
                ms = MotionState(int(row.values[KeyEnum.time_stamp_ms]))
                ms.x = float(row.values[KeyEnum.x])
                ms.y = float(row.values[KeyEnum.y])
                ms.vx = float(row.values[KeyEnum.vx])
                ms.vy = float(row.values[KeyEnum.vy])
                ms.psi_rad = float(row.values[KeyEnum.psi_rad])
                track.motion_states[ms.time_stamp_ms] = ms
        np.save(root_dir+'\\maps\\'+self.args.map_name+'\\track.npy', track_dict)
    ...
    def track_init(self):
        ...
        """Track loading"""
        path='./maps/'+self.args.map_name+'/track.npy'
        if not os.path.exists(path):
            print('generating the map file')
            self.generate_track_dict()

        self.track_dictionary = np.load(path,allow_pickle=True).item()
...
if __name__ == "__main__":
    ...
    pysim = Env(args)
    for i in range(2000):
        state, reward, done, _ = pysim.step(3)
        if done == True:
