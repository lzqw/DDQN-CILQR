...
class Constraints:
	def __init__(self, args, obstacle_bb):
		...
		self.number_of_npc = self.args.K # hardcode
	...
	def set_k(self,k):
		self.number_of_npc = k
		for i in range(self.number_of_npc):
			self.obs_constraints[i] = Obstacle(self.args, i, self.obstacle_bb)
	...
	def get_state_cost_derivatives(self, state, poly_coeffs, x_local_plan, npc_traj):
		...
		for i in range(self.args.horizon):
			...
			for j in range(self.number_of_npc):
				# pdb.set_trace()
				# (2, 4, 21)
				if npc_traj.shape[0]!=0:
					b_dot_obs, b_ddot_obs = self.obs_constraints[j].get_obstacle_cost_derivatives(npc_traj[j,:,:], i, state[:, i])
				else:
