...
class Obstacle:
    ...
    def get_obstacle_cost_derivatives(self, npc_traj, i, ego_state):
        ...
        ego_front = ego_state + np.array([math.cos(theta_ego)*self.args.ego_lf,
                                          math.sin(theta_ego)*self.args.ego_lf,
                                          0, 0])
        # diff = (transformation_matrix @ (ego_front - npc_traj[:, i])).reshape(-1, 1)
        diff = (ego_front - npc_traj[:, i]).reshape(-1, 1)
        c = 1 - diff.T @ P1 @ diff
        c_dot = -2 * P1 @ diff
        ...
        diff = (ego_rear - npc_traj[:, i]).reshape(-1, 1)
        c = 1 - diff.T @ P1 @ diff
        c_dot = -2 * P1 @ diff
